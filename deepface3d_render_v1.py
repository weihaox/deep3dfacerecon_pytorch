...
def numpy_dict_to_tensor_dict(pred_coeffs):
    """
    Return:
        pred_coeffs_dict    -- a dict of torch.tensors
    Parameters:
        pred_coeffs         -- a dict of numpy arrays
    """
    pred_coeffs_dict = {}
    for key in pred_coeffs:
        # from_numpy() automatically inherits input array dtype;
        # torch.Tensor is an alias for torch.FloatTensor.
        pred_coeffs_dict[key] = torch.from_numpy(pred_coeffs[key])
...
if __name__ == '__main__':
    ...
    # pred_coeffs_dict = numpy_dict_to_tensor_dict(pred_coeffs)
    coeffs = merge_coeff(pred_coeffs_dict) # torch.Size([1, 257])
    ...
    pred_face_numpy = pred_face_numpy[0] # (224, 224, 3)
    util.save_image(pred_face_numpy, mat_fname.replace('mat', 'png'))
